...
CIRCUIT_POSEIDON_BYTES_DIR="$PROVEKIT_DIR/noir-examples/poseidon-perm-bytes"
CIRCUIT_KECCAK_PERM_DIR="$PROVEKIT_DIR/noir-examples/keccak-f-perm"
CIRCUIT_KECCAK_BYTES_DIR="$PROVEKIT_DIR/noir-examples/keccak-bytes"
...
# Poseidon-bytes measurements (byte-equivalent)
for log in "${POSEIDON_SIZES[@]}"; do
  echo "--- Poseidon-bytes LOG=$log ---"
  scheme_file="$CIRCUIT_POSEIDON_BYTES_DIR/noir-proof-scheme-bytes_${log}.nps"
  inputs_file="$CIRCUIT_POSEIDON_BYTES_DIR/Prover_${log}.toml"
  if [[ ! -f "$scheme_file" || ! -f "$inputs_file" ]]; then
    echo "Error: Missing scheme or inputs for LOG=$log: $scheme_file / $inputs_file" >&2
    exit 1
  fi

  out_file="$RESULTS_DIR/provekit_poseidon_bytes_log${log}.json"
  echo "Measuring proving RAM..."
  (
    cd "$CIRCUIT_POSEIDON_BYTES_DIR"
    "$MEASURE_SCRIPT_ABS" --json "$ROOT_DIR/$out_file" -- \
      cargo run --release --bin noir-r1cs prove "./noir-proof-scheme-bytes_${log}.nps" "./Prover_${log}.toml" -o "./noir-proof-bytes_${log}.np"
  )
  echo "Saved: $out_file"
done
...
# Keccak-bytes measurements (byte-equivalent)
for log in "${KECCAK_SIZES[@]}"; do
  echo "--- Keccak-bytes LOG=$log ---"
  scheme_file="$CIRCUIT_KECCAK_BYTES_DIR/noir-proof-scheme_${log}.nps"
  inputs_file="$CIRCUIT_KECCAK_BYTES_DIR/Prover_${log}.toml"
  if [[ ! -f "$scheme_file" || ! -f "$inputs_file" ]]; then
    echo "Error: Missing scheme or inputs for LOG=$log: $scheme_file / $inputs_file" >&2
    exit 1
  fi

  out_file="$RESULTS_DIR/provekit_keccak_bytes_log${log}.json"
  echo "Measuring proving RAM..."
  (
    cd "$CIRCUIT_KECCAK_BYTES_DIR"
    "$MEASURE_SCRIPT_ABS" --json "$ROOT_DIR/$out_file" -- \
      cargo run --release --bin noir-r1cs prove "./noir-proof-scheme_${log}.nps" "./Prover_${log}.toml" -o "./noir-proof_${log}.np"
  )
  echo "Saved: $out_file"
