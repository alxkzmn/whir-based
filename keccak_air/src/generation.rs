...
pub fn generate_trace_rows<F: PrimeField64>(
    ...
    rows.par_chunks_mut(NUM_ROUNDS)
        .zip(padded_inputs)
        .for_each(|(row, input)| {
            // Initialize selectors for this permutation chunk.
            // We'll set first_row_sel and last-row transition after the parallel section.
            for r in row.iter_mut() {
                r.first_row_sel = F::ZERO;
                r.transition_sel = F::ONE;
            }

            generate_trace_rows_for_perm(row, input);
        });

    // After all chunks are generated, set global selectors:
    // - Set the very first row's first_row_sel = 1
    // - Set the very last row's transition_sel = 0
    if let Some(first) = rows.first_mut() {
        first.first_row_sel = F::ONE;
    }
    if let Some(last) = rows.last_mut() {
        last.transition_sel = F::ZERO;
    }
