...
use crate::{generate_trace_rows, BITS_PER_LIMB, NUM_ROUNDS, NUM_ROUNDS_MIN_1, U64_LIMBS};
...
impl<AB: AirBuilder> Air<AB> for KeccakAir {
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let gate_transition_not_final =
            not_final_step.clone() * local.transition_sel.clone().into();
        for y in 0..5 {
            for x in 0..5 {
                builder.assert_zeros::<U64_LIMBS, _>(array::from_fn(|limb| {
                    gate_transition_not_final.clone()
        ...
        builder.assert_zero(not_final_step.clone() * local.export.clone());
