...
pub struct KeccakCols<T> {
    ...
    /// Selector: 1 on the very first row of the entire trace, 0 elsewhere.
    pub first_row_sel: T,

    /// Selector: 1 on rows that have a valid next row (all except the last global row).
    pub transition_sel: T,
